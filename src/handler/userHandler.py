...
from src.models.sys import User, Role
    ...
    def post(self):
        try:
            User.update(nickname=self.get_json_body("nickname"), email=self.get_json_body("email"),
                        describe=self.get_json_body("describe"), update_at=datetime.now()).where(
                User.user_id == self.get_json_body("userId")).execute()
...
class UserGet(BaseHandler):
    ...
    def get(self):
        try:
            ...
            role_data = Role.select().where(Role.role_id == user_data.role_id).get()
            result = {"userId": user_data.user_id, "username": user_data.name, "nickname": user_data.nickname,
                      "email": user_data.email, "roleName": role_data.name, "describe": user_data.describe}
